...
| `VEN-C8` | Venus materializes Eon-authored ordered horizontal tabs and every fitting per-active-tab accordion pane header around exactly one expanded pane. For each visible live endpoint it observes Orbit's bounded read-only metadata and projects one bounded title/CWD label through both the existing clipped header and AccessKit name; empty, offline, unavailable, and incompatible metadata fall back honestly to pane identity without displaying mapped Session identity. Hidden tabs have no observers, and topology, endpoint, liveness, active-tab, workspace-loss, or Orbit-exit changes retire obsolete observations. Pointer, Alt+H/L tab traversal, Alt+K/J pane traversal, Alt+M pane creation, Ctrl+T tab creation, focused arrow traversal, and the selected terminal attachment remain on their existing owners. Venus re-inspects Eon every 250 ms so external accepted workspace changes appear without native input, while owning neither topology, terminal state, nor Session lifetime. | Proved | Eon `EON-C10`, EONW v1, and `eon-workspace-protocol` 0.1.0 at exact producer proof `4af395aea06c230ee6b18cf0755ae25915c0b88d`; canonical ORBS v5 and `orbit-protocol` 0.1.0 at accepted Orbit proof `69c402737799f03e615473956954a043647a4713` | Venus `3612a929402de00d07d582b6547d99f66afe5cad`; clean locked checks pass 61 library, 28 host, and 14 canonical integration tests with Clippy warnings denied. Focused checks cover exact read-only negotiation and ordering, latest-only coalescing, immediate observer release, active-tab and live-endpoint filtering, selected-attachment negotiation ordering, bounded safe labels, clipping geometry at scales 1.0, 1.25, 1.5, and 2.0, and one shared visual and AccessKit label. Isolated native COSMIC Wayland dogfood on Linux 7.0.11 at 1920×1080, 144 Hz, and scale 1.0 used exact Orbit `69c402737799f03e615473956954a043647a4713` and EONW producer `4af395aea06c230ee6b18cf0755ae25915c0b88d`. Four disposable Sessions showed distinct visible metadata, alternating CWD, endpoint replacement, active-tab and offline retirement, workspace-loss cleanup, bounded long labels, half-width layout, matched selected native title, interactive input, and unchanged Orbit processes through Venus and EONW loss. Dogfood caught and corrected a startup negotiation race before this proof revision. Fresh-eyes correction `3612a929402de00d07d582b6547d99f66afe5cad` invalidates the existing presentation exactly when metadata values or visible observer membership change. On exact correction source, an isolated native COSMIC Wayland probe with a static selected pane and hidden cursor captured the inactive header advancing from `inactive-263` to `inactive-265` in 163 ms; no unrelated terminal generation or blink state changed. Existing workspace topology, refresh, navigation, and capture proofs remain unchanged. | Fractional native scale and native AT-SPI inspection were not rerun; deterministic scale geometry and exact shared AccessKit-name checks cover the changed projection. Other compositors remain outside the accepted platform. |
...
`VEN-C8` removes transport decoration only from unambiguous local working
directories: `file://localhost/path` and `file:///path` display as `/path`.
Other authorities and values remain literal before the existing safety bound.
Projection source `ae28346760253442fb4b54a97f57eea8d080b62e` changes the natively
dogfooded predecessor `f4862273f53baf8e8d22521c68bfa7d1236d4915` only by
this projection; focused red/green checks and the full locked suite prove it.
