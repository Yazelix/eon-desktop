...
product boundary, then displays only the final path component. The normalized
path equal to `HOME` displays the same U+F015 marker used by Nova home tabs;
root remains `/`, and empty values fall back to the exact pane identity.
