...
components. Two ASCII spaces separate pane identity from location. Root remains
`/`, and empty values fall back to the exact pane identity.
...
`4abd14194bec6a8e3a95b5186b6c86652eeef898` keeps the same metadata, header,
...
location. The existing 80-character bound elides leftmost components and
retains control-byte replacement. Clean locked format, check, 61 library, 28
host, 14 canonical integration tests, warnings-denied Clippy, and diff checks
pass on x86_64 Linux. Phase: mechanically verified; composed native dogfood
remains pending.
